...
BASETGZ="${ROOTFS_REP}/\$NAME-base.tgz"
DISTRIBUTION="\$DIST"
BUILDRESULT="${DEB_REP}/${OS}/${DIST}"
...
if [[ ! -f ${ROOTFS_REP}/$NAME-base.tgz ]];then
...
config_pbuilder ${DISTRO} ${VERSION} ${TYPE} ${ROOTFS_REP} ${DEB_REP}
...
for p in ${PACKAGES};do
	...
	case $Type in
		dsc)
			...
			dget -ux $Url
			cd ${Rep}
			OS=${DISTRO} DIST=${VERSION} ARCH=${TYPE} pdebuild
			;;
		git)
			echo "Git"
			;;
